...
def F(x):
    ...
    Deficit_energy1, Deficit_power1, Deficit1, DischargePH1, DischargeB1 = Reliability(S, hydro=baseload) # Sj-EDE(t, j), MW
    ...
    Deficit_energy, Deficit_power, Deficit, DischargePH, DischargeB = Reliability(S, hydro=np.ones(intervals) * CHydro.sum() * pow(10,3))
    ...
    GHydro = resolution * Deficit1.sum() / efficiencyPH / years
    ...
    cost = factor * np.array([sum(S.CPV), GInter * pow(10,-6), sum(S.CPHP), S.CPHS] + list(CDC) + [sum(S.CPV), GHydro * pow(10, -6), GPHES, 0, 0]) # $b p.a.
    cost = cost.sum()
...
#    ub = [10000.]   * pzones + [300]    * wzones + [10000.] * nodes + [10000.] * nodes  + [100000.] + [100000] + [1000.] * inters + [50.] * nodes
    ...
    ub = [10000.]   * pzones + [10000.] * (nodes - inters) + inters * [0]   + [100000.] + [1000.] * inters
